...
public class Mnemonic {
    ...
    public init(strength: Int = 128, wordlist: [String] = Wordlists.english) {
        ...
        for i in 0..<(bits.count / 11) {
            let wi = Int(bits[bits.index(bits.startIndex,
                                         offsetBy: i * 11)..<bits.index(bits.startIndex, offsetBy: (i + 1) * 11)],
                         radix: 2)!
    ...
    public static func toMnemonic(_ bytes: [UInt8], wordlist: [String] = Wordlists.english) throws -> [String] {
        ...
        var phrase = [String]()
        for i in 0..<(bits.count / 11) {
            let wi = Int(bits[bits.index(bits.startIndex, offsetBy: i * 11)..<bits.index(
                bits.startIndex,
                offsetBy: (i + 1) * 11)], radix: 2
                )!
            phrase.append(String(wordlist[wi]))
        }
        return phrase
    ...
    public static func toEntropy(_ phrase: [String], wordlist: [String] = Wordlists.english) throws -> [UInt8] {
        ...
        let entropyBytes = regex.matches(in: entropyBits,
                                         options: [],
                                         range: NSRange(location: 0, length: entropyBits.count)).map {
    ...
    public static func deriveChecksumBits(_ bytes: [UInt8]) -> String {
        ...
        let hash = Mnemonic.sha256(data: Data(bytes: bytes, count: bytes.count))
